...
Attribute ボタン⑥生徒用操作画面シートを複製するプログラム.VB_ProcData.VB_Invoke_Func = " \n14"
    ...
    Dim TMS表反映 As String: TMS表反映 = 生徒用ws.Range("L14") 'Worksheets("2操作画面（編集厳禁）")からTMS表反映結果をコピー（VBA画面でのみ変更可能）
    Dim シート検索 As Worksheet
    
    If 名前 = "" Then 'もし生徒名が入力されていない場合、エラーコードを表示
        MsgBox "生徒名を入力してください。"
        Exit Sub
    ElseIf Not TMS表反映 = "完了" Then
        MsgBox "ボタン⑤を押してTMS画面に結果を反映してください。"
        Exit Sub
